...
# ============================================================
# GATE 0: AUDITOR GUARDIAN — Forces OO activation
# ============================================================

AUDITOR_CHECKPOINT=".claude/AUDITOR_CHECKPOINT"
if [ ! -f "$AUDITOR_CHECKPOINT" ]; then
  log "${RED}⛔ HARD STOP: OCULUS-OMNIVIDENS AUDITOR NOT DETECTED.${NC}"
  log "   You have neglected to activate the mandatory session supervisor."
  log "   Workflow: open skill oculus-omnividens → Perform preflight → Issue verdict."
  log "   Without a verified Auditor Checkpoint, all gates are LOCKED."
  exit 1
fi

# Verify Auditor Verdict
if grep -q "VERDICT: FAIL" "$AUDITOR_CHECKPOINT"; then
  log "${RED}⛔ HARD STOP: AUDITOR VERDICT IS 'FAIL'.${NC}"
  log "   The session auditor has blocked this work due to defects."
  log "   Fix the blockers identified in the Auditor report before running gates."
  exit 1
fi

# ── Infrastructure Mutation Probe ───────────────────────────
log "  Inspecting for unauthorized Bare Metal Mutations..."
# Scan for deployment/write patterns bypasses:
# 1. Direct Docker management
# 2. Writing to files via tee/>/chmod
# 3. Syncing code blocks (scp/rsync)
MUTATION_DETECTED=$(grep -rE "docker run|docker-compose|scp .*@|rsync .*@|ssh .* [^ ]*[>|\|\|tee\|chmod\|cp\|mv\|rm]" . --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=docs --exclude-dir=.cursor --exclude="*.md" --exclude="*.mdc" 2>/dev/null | grep -vE "coolify|cloudflare" || true)

if [ -n "$MUTATION_DETECTED" ]; then
  log "${RED}⛔ CRITICAL VIOLATION: BARE METAL MUTATION DETECTED.${NC}"
  log "   Unauthorized write/deploy actions found in the codebase:"
  echo "$MUTATION_DETECTED" | head -5 | while read -r line; do log "     - $line"; done
  log "   Inspection (ls, tail, cat) is permitted. WRITING is FORBIDDEN."
  log "   Use the Coolify/Cloudflare pipeline or request explicit Permission."
  exit 1
fi
pass "Gate 0 — Auditor Guardian active (Read-Only Inspection allowed)"

  # Check: Port Sovereignty (4000s or 443 only for Backends)
  PORT_VIOLATION=$(grep -rE ":8000|:5000|:3000" . --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=docs --exclude-dir=.cursor --exclude="*.md" --exclude="*.mdc" 2>/dev/null || true)
  if [ -n "$PORT_VIOLATION" ]; then
    log "${RED}⛔ CRITICAL VIOLATION: PORT SOVEREIGNTY BREACHED.${NC}"
    log "   Non-compliant Ports found (8000/5000/3000 are RESERVED; use 4000 series)."
    log "   Violation: $PORT_VIOLATION"
    exit 1
  fi
  pass "Gate 0.1 — Port Sovereignty verified (4000-series compliant)"

function check_progress() {
  local gate_id=$1
  local current_errors=$2
  local manifest_file=".claude/FAILED_MANIFEST_${gate_id}"

  if [ -f "$manifest_file" ]; then
    local last_errors=$(cat "$manifest_file")
    if [ "$current_errors" == "$last_errors" ]; then
      log "${RED}⛔ STOP WORK: DECEPTIVE CONDUCT DETECTED.${NC}"
      log "   You are claiming progress, but the error manifest is IDENTICAL."
      log "   Thou shalt not bypass gates by lying about pre-existing errors."
      log "   Fix the actual errors before running this gate again."
      exit 1
    fi
  fi
  echo "$current_errors" > "$manifest_file"
}
...
# Gate 1b: eslint
log "  Running Gate 1b: eslint..."
LINT_CMD="npx eslint . --ext .ts,.tsx,.js,.jsx --ignore-path .eslintignore --max-warnings=0"
LINT_ERRORS=$($LINT_CMD 2>&1 || true)
if [ -n "$LINT_ERRORS" ]; then
  check_progress "1b" "$LINT_ERRORS"
  log "${RED}  [FAIL] Gate 1b — eslint errors found${NC}"
  echo "$LINT_ERRORS" | head -20
  exit 1
fi
pass "Gate 1b — no lint errors"
